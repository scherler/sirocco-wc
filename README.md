...
- **Using a sirocco-wc Lit component from a React app** (see `./examples/lit-in-react/README.md` in the repository)
  — the prop and event boundary, Shadow DOM style isolation, and npm-package vs.
  dist-bundle distribution, with a runnable proof-of-concept: a component
  generated by `sirocco-wc add` plus a Vite + React app that renders it. This example
  is included in the repository for developers but excluded from the published npm package.
